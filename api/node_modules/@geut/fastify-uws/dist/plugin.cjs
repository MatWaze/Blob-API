...
function fastifyUws(fastify, opts, next) {
  ...
  const websocketServer = new (0, _chunkGNQW3RZHcjs.WebSocketServer)(options);
  server[_chunkGNQW3RZHcjs.kWs] = websocketServer;
  ...
  fastify.addHook("onRoute", (routeOptions) => {
    ...
    if (wsOptions.topics) {
      wsOptions.topics.forEach((topic) => {
        topics[topic] = _chunkGNQW3RZHcjs.WebSocket.allocTopic(namespace, topic);
      });
    }
    routeOptions.handler = function(request, reply) {
      ...
      if (requestRaw[_chunkGNQW3RZHcjs.kWs]) {
        reply.hijack();
        const uRes = requestRaw.socket[_chunkGNQW3RZHcjs.kRes];
        requestRaw.socket[_chunkGNQW3RZHcjs.kWs] = true;
        ...
        uRes.upgrade(
          {
            ...
            handler: (ws) => {
              request.uws = true;
              const conn = new (0, _chunkGNQW3RZHcjs.WebSocket)(namespace, ws, topics);
              let result;
              try {
              ...
              if (result && typeof result.catch === "function") {
                result.catch(
                  (err) => errorHandler.call(this, err, conn, request)
                );
              }
            }
          },
          ...
          requestRaw[_chunkGNQW3RZHcjs.kWs]
        );
      } else {
