...
import { BlockTag, ethers, EventLog } from "ethers";
...
document.getElementById("transfered-from-owner")!.addEventListener("submit", async (e) =>
{
  e.preventDefault();

  const address = (document.getElementById("address2") as HTMLInputElement).value.trim();
  const status = document.getElementById("status")!;
  const deploymentBlock = 43138084;
  console.log(address);
  ...
  const filter = contract.filters.Transfer(wallet.address, address);
  const latestBlock = await provider.getBlockNumber();
  ...
  console.log(total);
  const transferedEl = document.getElementById("transfered");
  if (transferedEl) {
    transferedEl.textContent = `Transfered from owner: ${total.toString()} tokens`;
  }
});
